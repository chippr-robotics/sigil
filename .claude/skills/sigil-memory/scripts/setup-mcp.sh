...
install_mcp_server() {
    ...
    echo "📥 Installing MCP Logseq server..."
...
echo "🚀 Starting Logseq MCP Server..."
...
if [[ ! -d "\$GRAPH_PATH" ]]; then
    echo "❌ Graph directory not found: \$GRAPH_PATH"
    exit 1
fi
...
EOF
    ...
    echo "✅ Startup script created: $script_path"
}
...
configure_claude_integration() {
    ...
    echo "🔧 Configuring Claude integration..."
...
echo "🧪 Testing MCP Logseq Integration"
echo "Server: $MCP_URL"
...
main() {
    ...
    echo "🔧 Setting up MCP Integration with Logseq"
    echo "Graph: $graph_path"
    ...
    if [[ ! -d "$graph_path" ]]; then
        echo "❌ Graph path does not exist: $graph_path"
    ...
    echo "  • MCP Server: $install_dir/mcp-logseq/"
    echo "  • Claude Config: ~/.claude/mcp-config.json"
    echo "  • Startup Script: $install_dir/start-mcp-logseq.sh"
}
