...
echo "2. Review:"
echo -e "   ${BLUE}git diff Cargo.toml Cargo.lock CHANGELOG.md${NC}"
echo "3. Commit on a branch and open a PR against staging:"
echo -e "   ${BLUE}git checkout -b release/v$NEW_VERSION${NC}"
echo -e "   ${BLUE}git add Cargo.toml Cargo.lock CHANGELOG.md${NC}"
echo -e "   ${BLUE}git commit -m \"chore: bump version to $NEW_VERSION\"${NC}"
echo -e "   ${BLUE}git push -u origin release/v$NEW_VERSION${NC}"
echo "4. After it merges and staging reaches main, tag main:"
echo -e "   ${BLUE}git checkout main && git pull${NC}"
echo -e "   ${BLUE}git tag -a v$NEW_VERSION -m \"Release version $NEW_VERSION\"${NC}"
echo -e "   ${BLUE}git push origin v$NEW_VERSION${NC}"
echo ""
echo -e "${YELLOW}The tag push is what starts release.yml, and it has to come from${NC}"
echo -e "${YELLOW}a human credential. Nothing before step 4 cuts a release.${NC}"
